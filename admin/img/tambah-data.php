...
{
  // Logic Tambah data FIle
  $direktori = "img/ikan/";
  $file_name = $_FILES['NamaFile']['name'];
  move_uploaded_file($_FILES['NamaFile']['tmp_name'], $direktori.$file_name);

  mysqli_query($koneksi, "INSERT INTO fish SET
...
<html lang="en">
  <head>
    ...
    <title>Kelola Data [Tambah Data] - Admin</title>
  ...
  <body>
    ...
    >
      <div class="container">
        <a class="fw-bold fs-3 text-decoration-none text-white" href="#"
      ...
      <form class="w-xl w-lg-50 mx-auto mt-5 p-4 border rounded" id="form-tambah" action="" method="post" enctype="multipart/form-data">
        <div class="row mb-3">
          ...
          <a class="btn btn-outline-dark mb-3 me-4 " href="kelola-data.php">
            ...
            <h1 class="d-inline-block fs-lg-5 fs-sm-2 fw-bold">Tambah Data</h1>
          </div>
        </div>
        <div class="mb-3">
          <label for="nama_ikan" class="form-label fs-5">Nama Ikan</label>
          <input type="text" class="form-control" id="nama_ikan" name="nama_ikan" required>
        </div>
        <div class="mb-3">
          <label for="harga_ikan" class="form-label fs-5">Harga Ikan (Kg)</label>
          <input type="number" class="form-control" id="harga_ikan" name="harga_ikan" required>
        </div>
        <div class="mb-3">
          <label for="stock_ikan" class="form-label fs-5">Stock Ikan</label>
          <input type="number" class="form-control" id="stock_ikan" name="stock_ikan" required>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <textarea class="w-100 form-control" name="deskripsi" id="deskripsi" row="4" cols="100"></textarea>
        </div>
        <div class="mb-3">
          <label for="gambar" class="form-label fs-5">Gambar Ikan</label>
          <input type="file" class="form-control" id="gambar" name="NamaFile" required>
        </div>
        <button type="submit" name="tambah" class="btn btn-outline-primary mt-3 w-100 fw-bold">Simpan</button>
</form>
      </div>
    </section>
